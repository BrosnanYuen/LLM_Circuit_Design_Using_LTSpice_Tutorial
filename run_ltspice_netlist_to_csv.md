...
```
2. Create netlist/editor object, MUST USE ABSOLUTE PATHS ONLY of the targeted `circuit.net`:
```json
...
```json
ltspice_mcp_execute {"api_name":"SimRunner.run","inputs":{"object_name":"runner","new_object_name":"run_task","netlist":"/path/to/folder/circuit.net","wait_resource":true}}
```
...
12. Export the targeted traces "V(opamp_input)" to CSV:
```json
